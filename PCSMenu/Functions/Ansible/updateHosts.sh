...
append_machine_to_existing_group() {
    ...
    # Find the last host entry under the specified group and insert the new machine there.
    # This requires adjusting the awk command to ensure it captures the correct insertion point.
    ...
    if [ -n "$last_host_line" ] && [[ "$last_host_line" -gt 0 ]]; then
        # Insert the new machine details after the last host entry.
        sed -i "$((last_host_line + 1))i \        $machine_name:\n          ansible_host: $ip_address\n          ansible_port: $port" "$HOSTS_FILE"
    else
        echo "Error: Could not determine the appropriate location to insert the new machine within '$group_name'."
        return 1
    fi
}
...
append_group_with_vars() {
    ...
    # Append new group with its first machine and vars
...
main() {
    ...
    if check_group_exists "$group_name"; then
        echo "Adding machine to existing group: $group_name"
        ...
        echo "Creating new group: $group_name"
...
append_group_with_vars_logic() {
    ...
    local auth_method
    local auth_value
    # The modified part for authentication method selection
    while true; do
        ...
        ssh)
            ...
            if [ -f "$auth_value" ]; then
                echo "SSH key for $machine_name already exists at $auth_value."
            else
                echo "SSH key for $machine_name does not exist at $auth_value."
                while true; do
                    ...
                    [Yy]*)
                        ...
                        ;;
                    [Nn]*)
                        echo "Please provide the $machine_name ssh private key content (paste below and press Ctrl-D when finished):"
                        cat >"$auth_value"
                        break
                        ;;
        ...
        password)
            ...
            break
            ;;
        *) echo "Invalid authentication method selected. Please answer ssh or password." ;;
        esac
    done
