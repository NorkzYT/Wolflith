...
read -p "Enter the parent directory location for where the 'Wolflith' folder is located: " parent_directory

# Use the 'find' command to search for the 'Wolflith' folder in the parent directory
directory_location=$(find $parent_directory -type d -name "Wolflith" | head -n 1)

# Check if the 'Wolflith' folder was found
if [[ -z "$directory_location" ]]; then
    echo "Error: Could not find 'Wolflith' folder in the sphecified parent directory."
    exit 1
fi
...
ansible-playbook "$directory_location/Wolflith/Ansible/playbooks/docker.yml" -i "$directory_location/Wolflith/Ansible/inventory/hosts"
...
ansible-playbook "$directory_location/Wolflith/Ansible/playbooks/docker-network.yml" -i "$directory_location/Wolflith/Ansible/inventory/hosts"
...
ansible-playbook "$directory_location/Wolflith/Ansible/playbooks/binfmt.yml" -i "$directory_location/Wolflith/Ansible/inventory/hosts"
...
ansible-playbook "$directory_location/Wolflith/Ansible/playbooks/first-time-docker-container-setup.yml" \
    -i "$directory_location/Wolflith/Ansible/inventory/hosts" \
